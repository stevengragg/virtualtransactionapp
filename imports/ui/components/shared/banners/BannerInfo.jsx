...
function BannerInfo({ children, className, type, open, setOpen, noClose }) {
  const typeIcon = (type) => {
    switch (type) {
      case "warning":
        return (
  ...
  const typeColor = (type) => {
    switch (type) {
      ...
      case "error":
        return "bg-rose-400 border-rose-200 text-rose-600 rounded-lg shadow-lg";
      case "success":
        ...
        <div className={className}>
          ...
          >
            <div className="flex w-full justify-between items-start">
              <div className="flex flex-wrap p-2 space-x-4 items-center align-middle">
                {children}
              </div>
              {!noClose && (
                <button
